...
_blend_argb(int count, const SW_FT_Span *spans, void *user_data)
{
   ...
   color = ECTOR_MUL4_SYM(sd->color, sd->mul_col);
   RGBA_Comp_Func_Solid comp_func = ector_comp_func_solid_span_get(sd->op, color);

   // move to the offset location
   buffer = sd->raster_buffer.buffer + ((pix_stride * sd->offy) + sd->offx);
...
_blend_alpha(int count, const SW_FT_Span *spans, void *user_data)
{
   ...
   // move to the offset location
   uint32_t *buffer =
         sd->raster_buffer.buffer + ((pix_stride * sd->offy) + sd->offx);
   uint32_t *mbuffer = mask->buffer;

   //Temp buffer for intermediate processing
   int tsize = sd->raster_buffer.width;
   ...
   while (count--)
     {
        ...
        uint32_t *mtarget =
              mbuffer + ((mask->w * spans->y) + spans->x);
...
_blend_gradient_alpha(int count, const SW_FT_Span *spans, void *user_data)
{
   ...
   Ector_Buffer *mask = data->mask;
   uint32_t *mbuffer = mask->buffer;

   //Temp buffer for intermediate processing
   uint32_t *tbuffer = malloc(sizeof(uint32_t) * data->raster_buffer.width);

   comp_func = ector_comp_func_span_get(data->op, data->mul_col, data->gradient->alpha);

   // move to the offset location
   buffer = data->raster_buffer.buffer + ((pix_stride * data->offy) + data->offx);
                                        ...
                                        int mask_op)
{
   ...
   rasterizer->fill_data.mask = mask;
