        ...
        if (last_it)
          {
             ...
             temp_w -= ellip_ti->parent.w;
              */
             temp_w += EVAS_TEXTBLOCK_TEXT_ITEM_SIZE(ellip_ti);
             ellip_ti = _layout_ellipsis_item_new(c, last_it);
             temp_w -= EVAS_TEXTBLOCK_TEXT_ITEM_SIZE(ellip_ti);
             /* END */
             c->x -= last_it->adv;
 ...
 */
static char *
_evas_textblock_cursor_range_text_markup_get(const Efl_Text_Cursor_Cursor *cur1, const Efl_Text_Cursor_Cursor *_cur2, Eina_Bool valid_markup)
/*******
 * END *
 *******/
{
