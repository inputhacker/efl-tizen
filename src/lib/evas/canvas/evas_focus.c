...
//        if (obj->layer->evas->focused) goto end;
        ...
        Evas_Event_Focus_In ev;
        if (obj->layer->evas->cur_seat)
          {
             ev.dev = obj->layer->evas->cur_seat;
             if (ev.dev) _evas_device_ref(ev.dev);
             evas_object_event_callback_call(eo_obj, obj, EVAS_CALLBACK_FOCUS_IN, &ev, event_id);
          }
        else
   ...
   else
     {
        ...
        Evas_Event_Focus_Out ev;
        if (obj->layer->evas->cur_seat)
          {
             ev.dev = obj->layer->evas->cur_seat;
             if (ev.dev) _evas_device_ref(ev.dev);
             evas_object_event_callback_call(eo_obj, obj, EVAS_CALLBACK_FOCUS_OUT, &ev, event_id);
          }
        else
