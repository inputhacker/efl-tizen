...
static void
_efl_vg_shape_render_pre(Evas_Object_Protected_Data *vg_pd,
                         Efl_VG *obj,
                         Efl_VG_Base_Data *nd,
                         ...
                         void *data)
{
   ...
   Efl_VG_Base_Data *fill, *stroke_fill, *stroke_marker;
   ...
   if (!nd->renderer)
     {
        eo_do(surface, nd->renderer = ector_surface_renderer_factory_new(ECTOR_RENDERER_GENERIC_SHAPE_MIXIN));
     }
...
static void
_efl_vg_shape_eo_base_destructor(Eo *obj, Efl_VG_Shape_Data *pd)
{
...
static void
_efl_vg_shape_efl_gfx_shape_commit(Eo *obj, Efl_VG_Shape_Data *pd)
{
...
{
   Eina_Bool ret = eo_do_ret(obj, ret, efl_gfx_shape_interpolate(from, to, pos_map));
   efl_canvas_vg_node_change(obj);
   return ret;
}
