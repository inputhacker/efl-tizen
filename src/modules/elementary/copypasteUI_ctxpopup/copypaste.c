...
#ifdef ENABLE_NLS
 #define S_SELECT dgettext("elementary", "IDS_COM_SK_SELECT")
 #define S_SELECT_ALL dgettext("elementary", "IDS_COM_BODY_SELECT_ALL")
 #define S_COPY dgettext("elementary", "IDS_COM_BODY_COPY")
 #define S_CUT dgettext("elementary", "IDS_COM_BODY_CUT")
 #define S_PASTE dgettext("elementary", "IDS_COM_BODY_PASTE")
 #define S_CLIPBOARD dgettext("elementary", "IDS_COM_BODY_CLIPBOARD")
...
_ctxpopup_position(Evas_Object *obj EINA_UNUSED)
{
   ...
   if (!ext_mod->have_selection)
     { //cannot get selection shape
        ...
        entry_text = edje_object_part_text_get(ext_mod->ent, "elm.text");

        //move to above or below cursor handler
        if (ch_visible && entry_text && strcmp(entry_text, ""))
          {
